...
export const WHY_EXTRACTION_SYSTEM_PROMPT = 'Extract concise changelog WHY notes from PR description candidates. Use only provided evidence. If the reason is unclear, indirect, or speculative, omit that PR. Return strict JSON.';
 ...
 */
export function buildWhyExtractionPrompt(input) {
    return {
        ...
        output: {
            ...
            confidence: 'Use high for explicit WHY/Reason/Motivation evidence, medium for clear context/problem evidence, and low for weak evidence.',
        },
 ...
 */
export function parseWhyExtractionOutput(rawText) {
    const parsedJson = extractJsonObject(rawText || '{"items":[]}');
