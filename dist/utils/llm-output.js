 ...
 */
export async function buildChangelogLlmOutput(params) {
    const fallbackReasons = [];
    if (params.noAi) {
        fallbackReasons.push('AI disabled by --no-ai');
    }
    if (params.requireProvider && !params.hasProviderKey) {
        throw new LlmError(`Provider required but API key is missing for provider: ${params.provider.name}`);
    }
    const hasCustomization = Boolean(params.customInstructions || params.language !== 'en');
    if (params.noAi || !hasCustomization || !params.hasProviderKey) {
        const fromRelease = await buildOutputFromReleaseNotes(params, fallbackReasons);
        if (fromRelease)
            return fromRelease;
    }
