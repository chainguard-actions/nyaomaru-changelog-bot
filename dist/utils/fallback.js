import { SECTION_ORDER, SECTION_ADDED, SECTION_CHANGED, SECTION_CHORE, SECTION_DOCS, SECTION_FIXED, SECTION_TEST, SECTION_REVERTED, SECTION_BREAKING_CHANGES, } from '../constants/changelog.js';
import { CONVENTIONAL_PREFIX_RE, INLINE_PR_NUMBER_RE, } from '../constants/conventional.js';
/**
 ...
 */
function formatBulletWithPrRef(title, prNumbers) {
 ...
 */
function detectBucket(subject) {
    const lower = subject.toLowerCase();
    if (TYPE_TO_SECTION_REGEX.Added.test(lower))
        return SECTION_ADDED;
    if (TYPE_TO_SECTION_REGEX.Fixed.test(lower))
        return SECTION_FIXED;
    if (TYPE_TO_SECTION_REGEX.Changed.test(lower))
        return SECTION_CHANGED;
    if (TYPE_TO_SECTION_REGEX.Docs.test(lower))
        return SECTION_DOCS;
    // Map build/ci to Chore (we do not emit a Build section)
    if (/^(build|ci)(\(|:)/i.test(lower))
        return SECTION_CHORE;
    if (TYPE_TO_SECTION_REGEX.Test.test(lower))
        return SECTION_TEST;
    if (TYPE_TO_SECTION_REGEX.Reverted.test(lower))
        return SECTION_REVERTED;
    if (BREAKING_MARKER_REGEX.test(subject))
 ...
 */
function extractPrNumbers(sha, subject, prMapBySha) {
...
export function fallbackSection(params) {
    ...
    if (!hasAnyItems)
        output.push('### Changed', '- Summary of changes', '');
    return output.join('\n');
}
