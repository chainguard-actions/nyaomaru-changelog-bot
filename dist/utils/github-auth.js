...
import { GitHubAccessTokenSchema, GitHubInstallationSchema, } from '../schema/github.js';
import { GITHUB_ACCEPT, GITHUB_API_VERSION, GITHUB_APP_JWT_ALG, GITHUB_APP_JWT_SKEW_SECONDS, GITHUB_APP_JWT_TTL_SECONDS, GITHUB_APP_SIGN_ALG, } from '../constants/github.js';
...
function createAppJwt(appId, privateKey) {
    ...
    const header = { alg: GITHUB_APP_JWT_ALG, typ: 'JWT' };
 ...
 */
export async function resolveGitHubAuth(owner, repo, config) {
    ...
    if (!installationId) {
        ...
        const inst = await getJson(instUrl, ghHeaders(jwt), 'GitHub API');
    ...
    const tokenRes = await postJson(tokenUrl, {}, ghHeaders(jwt), 'GitHub API');
