...
const ATX_HEADING_RE = new RegExp(`^(${ATX_HEADING_MARKER_RE_SOURCE})(?!#)\\s+(.*)$`);
const H2_HEADING_RE = new RegExp(`^${'#'.repeat(RELEASE_SECTION_HEADING_LEVEL)}\\s+(.*)$`);
const PREFIXED_ATX_HEADING_RE = new RegExp(`^([^\\p{L}\\p{N}#>*+-]+?)(${ATX_HEADING_MARKER_RE_SOURCE})(?!#)\\s+(.*)$`, 'u');
...
const MIN_NESTED_ADDITIONAL_SECTION_HEADING_LEVEL = CHANGELOG_ADDITIONAL_SECTION_HEADING_LEVEL + HEADING_LEVEL_STEP;
 ...
 */
function stripBulletPrefix(input) {
 ...
 */
function demoteAdditionalSectionHeadings(body) {
    let inFence = false;
    let fenceMarker;
    return body
        .split('\n')
        ...
        const nestedLevel = Math.min(Math.max(headingLevel + HEADING_LEVEL_STEP, MIN_NESTED_ADDITIONAL_SECTION_HEADING_LEVEL), MAX_MARKDOWN_ATX_HEADING_LEVEL);
 ...
 */
function stripTrailingByIn(input) {
...
function extractPr(text, repo) {
    ...
    if (refMatch) {
        const pr = Number(refMatch[REF_PAREN_PR_NUMBER_MATCH_INDEX] ||
 ...
 */
function collectH2Sections(body) {
 ...
 */
function isWhatsChangedHeading(heading) {
    const normalizedHeading = normalizeHeading(heading);
    return (normalizedHeading.startsWith("what's changed") ||
 ...
 */
function isFullChangelogHeading(heading) {
 ...
 */
function parseWhatsChangedLines(lines) {
    const collected = [];
    for (const rawLine of lines) {
        const line = rawLine.trim();
        if (!line || FULL_CHANGELOG_RE.test(line))
            continue;
        collected.push(line);
    }
    return collected;
 ...
 */
function parseReleaseLine(line, repo) {
    if (!line)
        return undefined;
 ...
 */
function toReleaseSection(section) {
    const filteredLines = section.lines.filter((line) => !FULL_CHANGELOG_RE.test(line));
 ...
 */
function extractFullChangelog(body, repo) {
 ...
 */
export function parseReleaseNotes(body, repo) {
    const items = [];
    ...
    for (const section of h2Sections) {
        if (isWhatsChangedHeading(section.heading))
            continue;
        if (isFullChangelogHeading(section.heading))
            continue;
...
export function buildSectionFromRelease(params) {
    ...
    const itemLookup = buildTitleLookup(items.map((item) => ({
        // WHY: LLM output can refer to either the stripped or raw release-note title.
        titles: new Set([item.title, item.rawTitle].filter(Boolean)),
        value: item,
    })));
    ...
    for (const section of sections) {
        lines.push(`${'#'.repeat(CHANGELOG_ADDITIONAL_SECTION_HEADING_LEVEL)} ${section.heading}`, '');
