import { BULLET_PREFIX_RE, H3_SUBHEADER_CAPTURE_RE, } from '../constants/markdown.js';
/**
 ...
 */
export function parseMarkdownSectionName(line) {
 ...
 */
export function hasMeaningfulMarkdownContent(lines) {
 ...
 */
export function appendUniqueBulletLines(section, bullets) {
    if (!bullets.length)
        return;
    const existingBullets = new Set(section.lines
        .filter((line) => BULLET_PREFIX_RE.test(line))
        .map((line) => line.trim()));
    const uniqueBullets = bullets.filter((line) => !existingBullets.has(line.trim()));
 ...
 */
export function renderMarkdownSections(document) {
    const output = [];
