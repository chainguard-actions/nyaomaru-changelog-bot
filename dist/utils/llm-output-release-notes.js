...
import { buildReleaseItemsFromPullRequests, parseReleaseNotes, buildSectionFromRelease, } from '../utils/release.js';
...
import { appendFallbackNote, buildAutoPrBody, buildPrUrl, resolvePrFromTitles, } from '../utils/llm-output-common.js';
/**
...
async function enrichReleaseItems(params) {
    ...
    for (const item of items) {
        ...
        if (item.pr) {
            ...
            if (!item.author) {
                try {
                    const pr = await fetchPRInfo(owner, repo, item.pr, token, githubApiBase);
                    if (pr?.author)
...
export async function buildOutputFromReleaseNotes(params, fallbackReasons) {
    const { owner, repo, version, date, prevRef, releaseRef, releaseBody, titleToPr, provider, hasProviderKey, noAi, failOnLlmError, token, githubApiBase, } = params;
    ...
    if (!parsedRelease.items.length && !parsedRelease.sections?.length) {
        parsedRelease.items = buildReleaseItemsFromPullRequests(params.commitList, params.pullRequestsBySha ?? {});
        usedPullRequestMetadata = parsedRelease.items.length > 0;
    }
    const hasAdditionalSections = Boolean(parsedRelease.sections?.length);
    if (!parsedRelease.items.length && !hasAdditionalSections)
        return null;
    fallbackReasons.push(usedPullRequestMetadata
