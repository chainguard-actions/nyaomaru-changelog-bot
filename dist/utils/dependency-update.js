...
const DEPENDABOT_BUMP_RE = /^bump\s+[@\w./-]+\s+from\s+v?\d+(?:\.\d+){0,3}\s+to\s+v?\d+(?:\.\d+){0,3}\b/i;
/**
 ...
 */
export function isDependencyUpdateTitle(rawTitle) {
    if (!rawTitle)
        return false;
    ...
    if (scope && DEP_SCOPE_RE.test(scope))
        return true;
    const lower = trimmedTitle.toLowerCase();
    if (DEP_BOT_RE.test(lower))
        return true;
    ...
    const hasVersionHint = VERSION_FROM_TO_RE.test(core) || DEP_VERSION_RE.test(core);
    if (hasDepPlural && hasAction)
        return true;
    if (hasDepSingular && hasAction && hasVersionHint)
        return true;
    if (DEPENDABOT_BUMP_RE.test(core))
        return true;
    return false;
}
