...
export function extractJsonObject(rawText) {
    const directParse = safeJsonParse(rawText);
 ...
 */
function tryParseOutermostSpan(text) {
    const firstBraceIndex = text.indexOf('{');
    const lastBraceIndex = text.lastIndexOf('}');
    if (firstBraceIndex === -1 || lastBraceIndex <= firstBraceIndex) {
        return undefined;
    }
    return safeJsonParse(text.slice(firstBraceIndex, lastBraceIndex + 1));
}
/**
 ...
 */
function tryParseBalancedObject(text) {
    const braceStack = [];
    let innermostValidObject;
    for (let index = 0; index < text.length; index++) {
        ...
        const candidate = safeJsonParse(text.slice(startIndex, index + 1));
