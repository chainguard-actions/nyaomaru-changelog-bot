...
import { RELEASE_HEADER_CAPTURE_RE, H3_SUBHEADER_CAPTURE_RE, } from '../constants/markdown.js';
/**
 ...
 */
function normalizeReleaseHeader(line) {
 ...
 */
function normalizeCategoryHeader(line) {
    const subHeaderMatch = line.match(H3_SUBHEADER_CAPTURE_RE);
    if (!subHeaderMatch)
        return line;
    const rawKey = subHeaderMatch[1];
    const normalizedKey = CATEGORY_NORMALIZE_MAP[rawKey.trim().toLowerCase()] ?? rawKey;
 ...
 */
export function normalizeSectionCategories(md) {
    if (!md)
        return md;
    const lines = md.split('\n');
    const normalized = lines.map((line, index) => index === 0 ? normalizeReleaseHeader(line) : normalizeCategoryHeader(line));
