...
import { SECTION_BREAKING_CHANGES, SECTION_CHANGED, SECTION_CHORE, SECTION_ORDER, } from '../constants/changelog.js';
import { BULLET_PREFIX_RE } from '../constants/markdown.js';
import { appendUniqueBulletLines, hasMeaningfulMarkdownContent, renderMarkdownSections, splitMarkdownSections, } from '../utils/markdown-sections.js';
const SECTION_ORDER_INDEX = new Map(SECTION_ORDER.map((sectionName, index) => [sectionName.toLowerCase(), index]));
function moveDependencyUpdatesToChore(markdown) {
    if (!markdown)
        return markdown;
    ...
    if (!sections.length)
        return markdown;
    const changedIndex = sections.findIndex((section) => section.name.toLowerCase() === SECTION_CHANGED.toLowerCase());
    if (changedIndex === -1)
        return markdown;
    ...
    if (!movedBullets.length)
        return markdown;
    changedSection.lines = retainedLines;
    if (!hasMeaningfulMarkdownContent(changedSection.lines)) {
        sections.splice(changedIndex, 1);
    }
    let choreSection = sections.find((section) => section.name.toLowerCase() === SECTION_CHORE.toLowerCase());
    if (!choreSection) {
        const choreOrder = SECTION_ORDER_INDEX.get(SECTION_CHORE.toLowerCase()) ??
            Number.POSITIVE_INFINITY;
        let insertIndex = sections.length;
        for (let i = 0; i < sections.length; i += 1) {
            const currentOrder = SECTION_ORDER_INDEX.get(sections[i].name.toLowerCase()) ??
                Number.POSITIVE_INFINITY;
...
function moveBreakingChangesToTop(markdown) {
    if (!markdown)
        return markdown;
    const document = splitMarkdownSections(markdown);
    const { sections } = document;
    if (!sections.length)
        return markdown;
    const isBreakingWithContent = (section) => section.name.toLowerCase() === SECTION_BREAKING_CHANGES.toLowerCase() &&
        hasMeaningfulMarkdownContent(section.lines);
    const breakingSections = sections.filter(isBreakingWithContent);
    if (!breakingSections.length)
        return markdown;
    const mergeBreakingLines = (blocks) => {
        const merged = [];
    ...
    if (!breakingAfterNonBreaking &&
        !hasMultipleBreaking &&
        isBreakingWithContent(sections[0]))
        return markdown;
    const mergedBreakingSection = {
 ...
 */
export function postprocessSection(markdown, titleToPr, repo) {
