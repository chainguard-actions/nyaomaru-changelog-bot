const PULL_URL_PR_NUMBER_RE = /\b(?<pullUrl>https?:\/\/[^\s)]+\/pull\/(?<prNumber>\d+)\b)/gi;
const PULL_LINK_SUFFIX_RE = /(?:\bin\s+)?\[#\d+\]\((?<pullUrl>https?:\/\/[^\s)]+\/pull\/(?<prNumber>\d+)\b[^)]*)\)(?:\s+by\s+@[\w-]+(?:\[bot\])?)?\s*$/i;
const PR_SUFFIX_RE = /(?:\bin\s+#(?<plain>\d+)\b|\(#(?<parenthesized>\d+)\))\s*$/i;
const AUTHOR_RE = /\sby\s@(?<author>[\w-]+(?:\[bot\])?)/i;
function cleanItemText(line, reference) {
...
function isCurrentRepositoryPullUrl(pullUrl, repository) {
    if (!pullUrl)
        return false;
    try {
        ...
        return (pathSegments[0]?.toLowerCase() === repository.owner.toLowerCase() &&
...
function parsePrNumber(rawNumber) {
    if (!rawNumber)
        return null;
    const prNumber = Number.parseInt(rawNumber, 10);
    return Number.isSafeInteger(prNumber) ? prNumber : null;
}
function extractOwningPrReference(line, repository) {
    const suffixMatch = PR_SUFFIX_RE.exec(line);
    const suffixPrNumber = parsePrNumber(suffixMatch?.groups?.plain ?? suffixMatch?.groups?.parenthesized);
    ...
    const linkedSuffixPrNumber = parsePrNumber(linkedSuffixMatch?.groups?.prNumber);
    if (linkedSuffixMatch &&
    ...
    const pullUrlMatches = Array.from(line.matchAll(PULL_URL_PR_NUMBER_RE)).filter((match) => isCurrentRepositoryPullUrl(match.groups?.pullUrl, repository));
 ...
 */
export function parseWhyBullet(line, repository) {
