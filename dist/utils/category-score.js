import { SECTION_ORDER, SECTION_ADDED, SECTION_CHANGED, SECTION_CHORE, SECTION_DOCS, SECTION_FIXED, SECTION_REVERTED, SECTION_TEST, SECTION_BREAKING_CHANGES, } from '../constants/changelog.js';
import { normalizeTitle } from '../utils/title-normalize.js';
import { FEAT_PREFIX_FLEX_RE, FIX_PREFIX_FLEX_RE, REFACTOR_PERF_STYLE_PREFIX_FLEX_RE, DOCS_PREFIX_FLEX_RE, TEST_PREFIX_FLEX_RE, REVERT_PREFIX_FLEX_RE, CHORE_PREFIX_FLEX_RE, PERF_PREFIX_FLEX_RE, } from '../constants/conventional.js';
import { BREAKING_PREFIX_MARKER_RE, COMBO_ADD_TO_IMPROVE_RE, COMBO_TIGHTEN_TYPE_RE, COMBO_FIX_BY_ADDING_RE, COMBO_REMOVE_WITHOUT_REPLACEMENT_RE, BUMP_OR_UPGRADE_RE, VERSION_FROM_TO_RE, } from '../constants/scoring.js';
import { isNullable } from '../utils/is.js';
import { BEST_CATEGORY_MIN_SCORE, BEST_CATEGORY_REQUIRED_MARGIN, CATEGORY_WEIGHTS, NEGATIVE_ATTENUATION_WEIGHT, NGRAM_MAX_WORDS, SCORE_MAX, SCORE_MIN, WEAK_KEYWORD_WEIGHT, WEIGHT, WEIGHT_LEVEL, } from '../constants/category-scoring.js';
 ...
 */
function addKeywordToIndex(index, section, entry, defaultWeight) {
    const { keyword, weight } = typeof entry === 'string'
 ...
 */
function buildKeywordIndex(groups, defaultWeight) {
    const index = new Map();
...
const STRONG_KEYWORD_INDEX = buildKeywordIndex([
    {
...
const WEAK_KEYWORD_INDEX = buildKeywordIndex([
 ...
 */
function hasBreakingMarkerInPrefix(rawTitle) {
...
function addScoreDeltas(scores, deltas) {
    for (const [sectionName, weight] of Object.entries(deltas)) {
 ...
 */
function createNormalizedPhrases(normalizedTitle) {
    ...
    for (let wordIndex = 0; wordIndex < words.length - 2; wordIndex++) {
        phrases.add(`${words[wordIndex]} ${words[wordIndex + 1]} ${words[wordIndex + 2]}`);
    }
    return phrases;
}
/**
 ...
 */
function collectPrefixFamilyDeltas(rawTitle, lowercasedTitle) {
    const deltas = {};
    ...
    if (REFACTOR_PERF_STYLE_PREFIX_FLEX_RE.test(lowercasedTitle)) {
        deltas[SECTION_CHANGED] = Math.max(CATEGORY_WEIGHTS.prefix.refactor, PERF_PREFIX_FLEX_RE.test(lowercasedTitle)
 ...
 */
function collectKeywordFamilyDeltas(normalizedPhrases, keywordIndex) {
    const deltas = {};
 ...
 * @param normalizedPhrases Normalized words and n-grams from the title.
 */
function applyNegativeSignalAttenuation(scores, normalizedPhrases) {
    const hasNegativeSignal = CATEGORY_WEIGHTS.negative.some(({ keyword }) => normalizedPhrases.has(keyword));
    ...
    if (bestSection) {
        scores[bestSection] = Math.max(scores[bestSection] - NEGATIVE_ATTENUATION_WEIGHT, SCORE_MIN);
    }
}
/**
 ...
 */
function applyComboHeuristics(scores, normalizedTitle) {
 ...
 */
function applyDependencyBumpHeuristic(scores, normalizedTitle) {
    if (!BUMP_OR_UPGRADE_RE.test(normalizedTitle))
        return;
    ...
    if (!Number.isNaN(fromMajorVersion) &&
 ...
 */
export function scoreCategories(rawTitle) {
    ...
    addScoreDeltas(scores, collectKeywordFamilyDeltas(normalizedPhrases, STRONG_KEYWORD_INDEX));
    addScoreDeltas(scores, collectKeywordFamilyDeltas(normalizedPhrases, WEAK_KEYWORD_INDEX));
 ...
 */
export function bestCategory(scores) {
    let topSection;
    let secondSection;
    for (const section of SECTION_ORDER) {
