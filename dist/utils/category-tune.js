...
export function tuneCategoriesByTitle(items, categories) {
    ...
    if (!adjusted[SECTION_FIXED])
        adjusted[SECTION_FIXED] = [];
    if (!adjusted[SECTION_CHANGED])
        adjusted[SECTION_CHANGED] = [];
    if (!adjusted[SECTION_ADDED])
        adjusted[SECTION_ADDED] = [];
    ...
    for (const title of nonDependencyTitles) {
        if (isImplicitFixTitle(title))
            toMove.push(title);
    }
    // Remove from any existing buckets and add to Fixed, ensuring uniqueness.
    if (toMove.length)
        moveTitlesToCategory(adjusted, toMove, SECTION_FIXED);
    ...
    const conventionalFixes = [];
    for (const title of nonDependencyTitles) {
        if (FIX_PREFIX_RE.test(title))
            conventionalFixes.push(title);
    }
    ...
    const isRefactorLike = (raw) => REFACTOR_LIKE_RE.test(raw.toLowerCase());
    const isChangeLike = (raw) => isChangeLikeTitle(raw);
    ...
    const toChanged = [];
    for (const title of nonDependencyTitles) {
        if (!isRefactorLike(title) && !isChangeLike(title))
            continue;
        const current = findCategory(title);
        if (current === SECTION_FIXED)
            continue; // don't override explicit/implicit fixes
    ...
    const toAdded = [];
    for (const title of nonDependencyTitles) {
        if (FEAT_PREFIX_RE.test(title))
            toAdded.push(title);
    }
    if (toAdded.length)
        moveTitlesToCategory(adjusted, toAdded, SECTION_ADDED);
