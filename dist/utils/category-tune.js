import { CONVENTIONAL_PREFIX_RE, FEAT_PREFIX_FLEX_RE, FIX_PREFIX_FLEX_RE, REFACTOR_LIKE_RE, } from '../constants/conventional.js';
import { SECTION_ADDED, SECTION_CHANGED, SECTION_FIXED, SECTION_CHORE, SECTION_BREAKING_CHANGES, SECTION_DOCS, SECTION_TEST, } from '../constants/changelog.js';
...
const WEAK_REMAP_BUCKETS = new Set([
    SECTION_CHORE,
    SECTION_DOCS,
    SECTION_TEST,
]);
function semanticTitleCore(rawTitle) {
    return rawTitle.toLowerCase().replace(CONVENTIONAL_PREFIX_RE, '').trim();
}
function collectKnownTitles(items, categories) {
    const knownTitles = new Set();
    for (const item of items) {
        if (item.rawTitle)
            knownTitles.add(item.rawTitle);
...
function cloneCategoryMap(categories) {
    const adjusted = Object.fromEntries(Object.entries(categories).map(([section, list]) => [
        section,
 ...
 */
function moveTitlesToCategory(adjusted, titles, targetCategory) {
    if (!Array.isArray(adjusted[targetCategory]))
        adjusted[targetCategory] = [];
...
function collectMatchingTitles(titles, predicate) {
    return titles.filter((title) => predicate(title));
}
function findCategory(categories, title) {
...
function shouldMoveChangeLikeTitle(title, currentCategory) {
    if (!REFACTOR_LIKE_RE.test(title.toLowerCase()) &&
        !isChangeLikeTitle(title)) {
        return false;
    }
    if (currentCategory === SECTION_FIXED)
        return false;
    if (currentCategory &&
 ...
 */
export function isImplicitFixTitle(rawTitle) {
    if (!rawTitle)
        return false;
    const core = semanticTitleCore(rawTitle);
    const mentionsType = TYPE_INTENT_INDICATORS.some((keyword) => core.includes(keyword));
    const impliesFix = FIX_INTENT_INDICATORS.some((keyword) => core.includes(keyword));
    // If the core mentions typing/contract and implies a correction, treat as Fixed.
    if (mentionsType && impliesFix)
        return true;
 ...
 */
export function isChangeLikeTitle(rawTitle) {
    if (!rawTitle)
        return false;
    const core = semanticTitleCore(rawTitle);
 ...
 */
export function tuneCategoriesByTitle(items, categories) {
    if (!items.length)
        return categories;
    ...
    const dependencyUpdates = collectMatchingTitles(allKnownTitles, isDependencyUpdateTitle);
    ...
    const nonDependencyTitles = allKnownTitles.filter((title) => !dependencyUpdateSet.has(title));
    const implicitFixes = collectMatchingTitles(nonDependencyTitles, isImplicitFixTitle);
    ...
    const conventionalFixes = collectMatchingTitles(nonDependencyTitles, (title) => FIX_PREFIX_FLEX_RE.test(title));
    ...
    const changeLikeTitles = nonDependencyTitles.filter((title) => shouldMoveChangeLikeTitle(title, findCategory(adjusted, title)));
    ...
    const featureTitles = collectMatchingTitles(nonDependencyTitles, (title) => FEAT_PREFIX_FLEX_RE.test(title));
