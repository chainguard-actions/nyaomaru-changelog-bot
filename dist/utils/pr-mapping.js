 ...
 */
function collectInlinePrHints(commitList) {
    const hints = {};
    for (const commit of commitList) {
        const numbers = extractPrRefsFromText(commit.subject);
        if (numbers.length)
            hints[commit.sha] = numbers;
    }
    return hints;
}
/**
 ...
 */
function mergePrNumbers(...sources) {
 ...
 */
function expandMergePrs(prsLog, repoPath, prMapBySha) {
    for (const line of prsLog.split('\n')) {
        ...
        if (!mergeSha || !numbers?.length)
            continue;
        const shasInMerge = commitsFromMerge(mergeSha, repoPath);
        for (const commitSha of shasInMerge) {
            prMapBySha[commitSha] = mergePrNumbers(numbers, prMapBySha[commitSha] ?? []);
        }
    }
}
/**
 ...
 */
export function buildTitleToPr(commitList, prsLog, prMapBySha) {
    const titleToPr = {};
    for (const commit of commitList) {
