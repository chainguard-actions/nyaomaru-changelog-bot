import { CONVENTIONAL_PREFIX_RE, REFACTOR_LIKE_RE, FIX_PREFIX_FLEX_RE, REFACTOR_PERF_STYLE_PREFIX_FLEX_RE, FEAT_PREFIX_FLEX_RE, } from '../constants/conventional.js';
...
import { SECTION_ADDED, SECTION_CHANGED, SECTION_FIXED, } from '../constants/changelog.js';
/**
 ...
 */
export function buildTitlesForClassification(items) {
    const out = [];
    ...
    for (const item of items) {
        ...
        if (REFACTOR_LIKE_RE.test(lower)) {
            out.push(REFACTOR_PERF_STYLE_PREFIX_FLEX_RE.test(lower)
                ? base
                : `refactor: ${core}`);
            continue;
        }
