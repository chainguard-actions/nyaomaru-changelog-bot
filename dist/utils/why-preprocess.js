...
function toCandidateSnippets(sections, body, maxCharsPerPr) {
    const sourceTexts = sections.length > 0 ? sections.map((section) => section.text) : [body];
    const snippets = [];
...
function scoreCandidateMaterial(sections, candidates, body) {
    ...
    if (RATIONALE_MARKER_RE.test(candidateText))
        score += 3;
    if (PROBLEM_MARKER_RE.test(candidateText))
        score += 2;
...
export function preprocessWhyPrBody(target, details, options) {
    ...
    const candidates = toCandidateSnippets(extracted.sections, body, options.maxCharsPerPr);
