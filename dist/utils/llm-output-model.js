...
import { appendFallbackNote, applyLlmDefaults, buildAutoPrBody, } from '../utils/llm-output-common.js';
import { LlmError } from '../lib/errors.js';
function buildLogsForLLM(commitList, prMapBySha) {
    return commitList
        .map((commit) => {
        const numbers = prMapBySha[commit.sha];
        const suffix = numbers?.length ? ` (#${numbers[0]})` : '';
        return `${commit.sha.slice(0, SHA_SHORT_LENGTH)} ${commit.subject}${suffix}`;
    })
        .join('\n');
}
export async function buildOutputFromModelOrFallback(params, fallbackReasons) {
    const { owner, repo, version, date, prevRef, releaseRef, releaseBody, language, customInstructions, existingChangelog, commitList, prs, prMapBySha, provider, hasProviderKey, noAi, failOnLlmError, } = params;
    ...
    if (!llm) {
        llm = {
            new_section_markdown: fallbackSection({
                ...
                date,
                logs: commitList
                    .map((commit) => `${commit.sha.slice(0, SHA_SHORT_LENGTH)} ${commit.subject}`)
                    .join('\n'),
                prs,
                prMapBySha,
            }),
