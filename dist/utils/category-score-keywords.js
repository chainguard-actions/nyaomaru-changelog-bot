import { SECTION_ADDED, SECTION_BREAKING_CHANGES, SECTION_CHANGED, SECTION_CHORE, SECTION_DOCS, SECTION_FIXED, SECTION_ORDER, SECTION_TEST, } from '../constants/changelog.js';
import { CATEGORY_WEIGHTS, NGRAM_MAX_WORDS, WEAK_KEYWORD_WEIGHT, WEIGHT, } from '../constants/category-scoring.js';
 ...
 */
export function createNormalizedPhrases(normalizedTitle) {
 ...
 */
export function collectKeywordScoreDeltas(normalizedPhrases) {
    const strongDeltas = collectKeywordFamilyDeltas(normalizedPhrases, STRONG_KEYWORD_INDEX);
    const weakDeltas = collectKeywordFamilyDeltas(normalizedPhrases, WEAK_KEYWORD_INDEX);
    const combinedDeltas = {};
    for (const section of SECTION_ORDER) {
        const combinedScore = (strongDeltas[section] || 0) + (weakDeltas[section] || 0);
        if (combinedScore !== 0)
            combinedDeltas[section] = combinedScore;
    }
