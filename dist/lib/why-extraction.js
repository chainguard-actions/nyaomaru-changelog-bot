...
import { WHY_MAX_TOTAL_PAYLOAD_CHARS, WHY_MIN_RENDER_TRUST_SCORE, } from '../constants/why.js';
import { preprocessWhyPrBody } from '../utils/why-preprocess.js';
import { applyWhyNotesToSection, extractWhyTargets, } from '../utils/why-targets.js';
...
function createEmptyDiagnostics(enabled) {
    return {
        enabled,
        aiUsed: false,
        targetsFound: 0,
...
function truncatePayloadItems(items) {
    const boundedItems = [];
    let usedChars = 0;
 ...
 */
function githubWebHost(apiBase) {
    try {
...
function appendWhyPreview(prBody, notes, whyLabel) {
    if (!notes.length)
        return prBody;
    const preview = notes
 ...
 */
async function collectWhyExtractionItems(params, targets, token) {
    const result = {
        items: [],
        prBodiesFetched: 0,
        skippedLowTrust: 0,
    ...
    for (const target of targets) {
        const details = await params.fetchPRDetails(params.owner, params.repo, target.prNumber, token, params.githubApiBase);
        if (!details) {
 ...
 */
function acceptWhyNotes(providerOutput, inputItems, minimumConfidence) {
    const itemsByPr = new Map(inputItems.map((item) => [item.prNumber, item]));
    const notes = [];
 ...
 */
export async function runWhyExtraction(params) {
    ...
    if (!params.hasProviderKey) {
        diagnostics.fallbackReasons.push(`WHY extraction skipped: missing API key for ${params.provider.name}`);
        return { llm, diagnostics };
    }
    if (cli.whyMaxPrs === 0) {
    ...
    const extractedTargets = extractWhyTargets(llm.new_section_markdown, repository);
    ...
    if (!targets.length) {
        diagnostics.fallbackReasons.push('WHY extraction skipped: no eligible changelog PRs');
        return { llm, diagnostics };
    }
    const collection = await collectWhyExtractionItems(params, targets, params.token);
    ...
    if (!boundedItems.length) {
        diagnostics.fallbackReasons.push('WHY extraction skipped: no trusted PR description candidates');
        return { llm, diagnostics };
    }
    let providerOutput;
    try {
    ...
    const llmAfterAiUse = prBodyAfterAiUse === llm.pr_body
        ? llm
        : { ...llm, pr_body: prBodyAfterAiUse };
    const accepted = acceptWhyNotes(providerOutput, boundedItems, cli.whyConfidence);
    diagnostics.skippedLowTrust += accepted.skippedLowTrust;
    const acceptedNotes = accepted.notes;
    if (!acceptedNotes.length) {
        diagnostics.fallbackReasons.push('WHY extraction skipped: provider returned no trusted notes');
        return { llm: llmAfterAiUse, diagnostics };
    }
    const notesByPr = new Map(acceptedNotes.map((note) => [note.prNumber, note]));
    diagnostics.notesRendered = acceptedNotes.length;
    return {
        llm: {
            ...llmAfterAiUse,
            new_section_markdown: applyWhyNotesToSection(llmAfterAiUse.new_section_markdown, notesByPr, cli.whyLabel, repository),
            pr_body: appendWhyPreview(llmAfterAiUse.pr_body, acceptedNotes, cli.whyLabel),
        },
        diagnostics,
    };
}
