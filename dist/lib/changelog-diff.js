 ...
 */
export function diffChangelog(oldText, newText) {
    ...
    for (let oldIdx = oldLen - 1; oldIdx >= 0; oldIdx--) {
        for (let newIdx = newLen - 1; newIdx >= 0; newIdx--) {
            if (oldLines[oldIdx] === newLines[newIdx]) {
                ...
                lcs[oldIdx][newIdx] = Math.max(lcs[oldIdx + 1][newIdx], lcs[oldIdx][newIdx + 1]);
            }
        }
    }
    // Reconstruct diff from the LCS matrix.
    const diffLines = [...header];
    const addContext = (line) => diffLines.push(' ' + line);
    const addRemoval = (line) => diffLines.push('-' + line);
    const addAddition = (line) => diffLines.push('+' + line);
    let oldIndex = 0;
    let newIndex = 0;
