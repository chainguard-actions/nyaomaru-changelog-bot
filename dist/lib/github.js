import { GitHubReleaseByTagSchema, GitHubPRInfoSchema, GitHubCommitPullsArraySchema, } from '../schema/github.js';
import { GITHUB_ACCEPT, GITHUB_API_BASE_DEFAULT, GITHUB_API_VERSION, PRS_LOOKUP_COMMIT_LIMIT, } from '../constants/github.js';
...
async function ghGet(url, token) {
    ...
    return getJson(url, requestHeaders, 'GitHub API');
}
/**
 ...
 */
export async function fetchReleaseBody(owner, repo, tag, token, apiBase = GITHUB_API_BASE_DEFAULT) {
...
export async function mapCommitsToPrs(owner, repo, shas, token, apiBase = GITHUB_API_BASE_DEFAULT) {
    ...
    for (const sha of shas.slice(0, PRS_LOOKUP_COMMIT_LIMIT)) {
        try {
            commitToPullRefs[sha] = await prsForCommit(owner, repo, sha, token, apiBase);
        }
        catch {
