 ...
 */
export function updateCompareLinks(content, compareLine, unreleasedLine) {
    let updatedContent = content;
    // Helper: does a line exist (ignoring trailing whitespace differences)?
    const containsLine = (line) => {
    ...
    if (unreleasedLine) {
        ...
        if (unreleasedLinkRegExp.test(updatedContent)) {
            updatedContent = updatedContent.replace(unreleasedLinkRegExp, unreleasedLine);
        }
