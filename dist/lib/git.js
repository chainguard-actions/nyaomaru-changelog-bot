...
function assertSafeGitRef(value, label) {
    if (!SAFE_REF_PATTERN.test(value) && !SAFE_SHA_PATTERN.test(value)) {
        throw new GitError(`Invalid ${label}: "${value}" contains unsupported characters.`);
    }
}
/**
 ...
 */
export function tryRun(args, cwd) {
    try {
        return run(args, cwd);
    }
    catch {
...
export function tryDetectLatestTag(cwd) {
 ...
 */
export function findPullRequestNumberByHeadSha(pullRefsOutput, headSha) {
 ...
 */
export function tryFindPullRequestNumberForBranch(branch, cwd) {
    const remoteHeadSha = tryRun(['rev-parse', `refs/remotes/origin/${branch}`], cwd);
    if (!remoteHeadSha)
        return null;
    const pullRefsOutput = tryRun(['ls-remote', 'origin', 'refs/pull/*/head'], cwd);
    if (!pullRefsOutput)
        return null;
    return findPullRequestNumberByHeadSha(pullRefsOutput, remoteHeadSha);
}
/** Detect the previous tag relative to the given tag. */
export function tryDetectPrevTag(currentTag, cwd) {
...
export function firstCommit(cwd) {
...
export function gitMergedPRs(from, to, cwd) {
...
export function commitsFromMerge(mergeSha, cwd) {
 ...
 */
export function commitsInRange(prev, next, repoPath = '.') {
 ...
 */
export function dateForRef(ref, cwd) {
 ...
 */
export function extractPrRefsFromText(text) {
