...
import { CliConfigFileSchema, } from '../schema/config-file.js';
 ...
 */
export function loadCliConfigFile(configPath, cwd = process.cwd()) {
    ...
    try {
        const rawConfig = JSON.parse(readFileSync(resolvedPath, CLI_CONFIG_ENCODING));
        return CliConfigFileSchema.parse(rawConfig);
    }
    catch (error) {
        if (error instanceof z.ZodError) {
            throw new ConfigError(`Invalid config file ${resolvedPath}: ${z.prettifyError(error)}`);
        }
