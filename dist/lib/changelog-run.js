...
import { resolveChangelogRunDependencies, } from '../lib/changelog-run-dependencies.js';
import { writeDryRunOutput, } from '../lib/changelog-dry-run.js';
...
export async function executeChangelogRun(params) {
    ...
    const { provider, releasePlan, mergedPullRequests, existingChangelog, commitList, token, hasProviderKey, pullRequestsBySha, releaseBody, prNumbersBySha, titleToPr, customInstructionsResolution, providerConfig, } = await resolveChangelogRunInput({
        cli,
        appConfig,
        deps,
    });
    const { owner, repo, changelogPath, releaseRef, version, prevRef, date } = releasePlan;
