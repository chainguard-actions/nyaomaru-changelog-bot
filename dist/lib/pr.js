...
import { DEFAULT_CHANGELOG_FILE, EXEC_OPTS, GIT_REMOTE, } from '../constants/git.js';
const INVALID_BRANCH_PUNCTUATION = ' ~^:?*[';
function runGit(args) {
    execFileSync('git', args, EXEC_OPTS);
}
function hasInvalidBranchChars(branchName) {
...
function assertSafeBranchName(branchName) {
    ...
    if (branchName.startsWith('-')) {
        throw new Error('Invalid branch name: branch name must not start with "-".');
    }
    if (branchName.startsWith('/') ||
        branchName.endsWith('/') ||
        branchName.includes('//') ||
    ...
    if (branchSegments.some((segment) => !segment ||
        segment.startsWith('.') ||
...
function assertSafeChangelogPath(changelogPath) {
    if (!changelogPath) {
...
export async function createPR(params) {
    const { owner, repo, baseBranch, branchName, title, body, labels, token, changelogEntry, } = params;
